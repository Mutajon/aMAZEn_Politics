...
const SystemSelection: React.FC<SystemSelectionProps> = ({ onSelect, onSuggestOwn }) => {
    return (
        <div className="fixed inset-0 flex flex-col items-center w-full px-4 pt-[24vh] pb-12 space-y-10 overflow-y-auto scrollbar-hide z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-4xl relative z-10">
                {FREE_PLAY_SYSTEMS.map((system, index) => (
                    <motion.div
                        ...
                        className="flex items-center gap-5 bg-white/5 border border-white/10 p-4 sm:p-5 rounded-[24px] cursor-pointer transition-all group backdrop-blur-sm hover:border-amber-400/40"
                    >
                        {/* Circle Image */}
